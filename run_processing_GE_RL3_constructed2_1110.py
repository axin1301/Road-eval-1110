...
def main(task):
    ...
    download_url = "https://anonymous.4open.science/api/repo/Road-eval-1110-5DC7/zip"

    if not os.path.exists("folder1110"):
        # 本地保存 ZIP 文件的路径
        zip_save_path = "folder1110.zip"
        extract_path = "folder1110"  # 解压到的文件夹
    ...
    source_folder = "folder1110/GraphSamplingToolkit-main_improve_GE_combined_full_four_kinds"

    # 指定目标文件夹路径
    destination_folder = "./GraphSamplingToolkit-main_improve_GE_combined_full_four_kinds"
    ...
    py_file_list = ['average_statistics_GE_RL3_constructed2_1110.py','main_pipeline_GE_RL3_constructed2_1110.py','main_pipeline_GE_RL3_constructed2_1110_single_test.py', \
                    'mapcompare_GE_RL3_constructed2_1110.py','mapcompare_GE_RL3_constructed2_1110_single_test.py']
    for py_file in py_file_list:
        shutil.copy('folder1110/'+py_file,'code1013/'+py_file)
    ...
    if task == 'test':
        python_script = "main_pipeline_GE_RL3_constructed2_1110_single_test.py"  # 替换为你要运行的 Python 脚本名
        print(f"运行 Python 脚本: {python_script}")
        subprocess.run(["python", python_script], check=True)

    else:
        # 第三步：运行指定的 Python 脚本
        python_script = "main_pipeline_GE_RL3_constructed2_1110.py"  # 替换为你要运行的 Python 脚本名
        print(f"运行 Python 脚本: {python_script}")
        subprocess.run(["python", python_script], check=True)
